...
async function handleOpenFolder() {
  try {
    ...
    currentDir.value = handle.name
    await loadDirectoryHandle(handle)
    emit('folderOpened', handle.name)
  } catch (err) {
...
async function loadDirectoryHandle(dirHandle: FileSystemDirectoryHandle, parentPath = '') {
  ...
  if (!parentPath) {
    topItems.value = entries
  } else {
    childItems.value.set(parentPath, entries)
  }
}
...
async function handleItemClick(item: FileInfo) {
  if (item.isDir) {
    if (expanded.value.has(item.path)) {
      ...
      // Nested directory loading would require keeping the directory handle.
      // For the web build we keep it shallow to avoid state complexity.
      childItems.value.set(item.path, [])
    }
